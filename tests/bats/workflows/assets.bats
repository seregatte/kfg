...
load '../helpers/vendor/bats-support/load'
load '../helpers/vendor/bats-assert/load'
load '../helpers/common'

# KFG is set via KFG_BIN in helpers/common.bash
...
@test "kfg apply --convert without --use fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert test-data
...
@test "kfg apply --use without --convert fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --use to-json
...
@test "kfg apply --convert/--use with -w fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert test-data --use to-json -w my-workflow
...
@test "kfg apply --convert/--use with -c fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert test-data --use to-json -c my-cmd
...
@test "kfg apply --convert with unknown asset fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert nonexistent --use to-json
...
@test "kfg apply --use with unknown converter fails" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert test-data --use nonexistent
...
@test "kfg apply --convert --use -o writes to file" {
  ...
  run $KFG_BIN apply -f "$TMPDIR/manifest.yaml" --convert test-data --use to-json -o "$TMPDIR/output.json"
