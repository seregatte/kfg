...
# ============================================================================
# Run Command -- Argument Forwarding Tests
# ============================================================================

@test "kfg run forwards args after -- to the agent (integration)" {
    # Create a manifest with a cmd that echoes forwarded arguments
    cat > "${TEST_TMPDIR}/echo-manifest.yaml" <<'EOF'
...
kind: Cmd
metadata:
  name: dev.cmds.echo-args
  commandName: echoArgs
spec:
  run: 'echo "FORWARDED_ARGS: $*"'
EOF

    # Run with forwarded args after --
    run "${KFG_BIN}" run -f "${TEST_TMPDIR}/echo-manifest.yaml" echoArgs -- hello world
    [ "$status" -eq 0 ]

    # Verify forwarded args were passed to the agent
    [[ "$output" =~ "FORWARDED_ARGS: hello world" ]]
}

@test "kfg run without -- does not pass extra args to the agent" {
    # Create a manifest with a cmd that echoes forwarded arguments
    cat > "${TEST_TMPDIR}/echo-manifest.yaml" <<'EOF'
apiVersion: kfg.dev/v1alpha1
kind: CmdWorkflow
...
EOF
    ...
    # Run without -- (extra args are not forwarded)
    run "${KFG_BIN}" run -f "${TEST_TMPDIR}/echo-manifest.yaml" echoArgs
    [ "$status" -eq 0 ]

    # Verify no args were forwarded (empty after the colon)
    [[ "$output" =~ "FORWARDED_ARGS: " ]]
}
