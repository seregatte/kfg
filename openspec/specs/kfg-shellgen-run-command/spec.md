...
- **AND** resolves the workflow containing the `opencode` cmd
- **AND** generates shell code
- **AND** executes the `opencode` agent function with inherited stdin/stdout/stderr
- **AND** exits with the agent's exit code

#### Scenario: Run with file
- **WHEN** user runs `kfg run -f manifest.yaml opencode`
- **THEN** loads manifest from file
- **AND** generates and executes the `opencode` agent

#### Scenario: Run with stdin
- **WHEN** user runs `kfg run -f - opencode`
- **THEN** reads manifest from stdin
- **AND** generates and executes the `opencode` agent

#### Scenario: Run with extra args
- **WHEN** user runs `kfg run -k .kfg/overlay/dev opencode -- --model gpt-4`
- **THEN** executes `opencode` with `--model gpt-4` as positional arguments
...
- **GIVEN** a Cmd with `metadata.name: dev.agents.opencode` and `metadata.commandName: opencode`
- **WHEN** user runs `kfg run opencode`
...
- **WHEN** user runs `kfg run opencode` without `-w` flag
...
- **WHEN** user runs `kfg run -k .kfg/overlay/dev opencode` without `-w`
- **THEN** searches all CmdWorkflows for one containing `dev.agents.opencode`
- **AND** uses the matching workflow

#### Scenario: Explicit workflow
- **WHEN** user runs `kfg run -k .kfg/overlay/dev -w dev opencode`
- **THEN** uses the specified `dev` workflow
- **AND** does not search other workflows

#### Scenario: Agent not in specified workflow
- **WHEN** user runs `kfg run -k path -w openspec opencode`
- **AND** `opencode` is not in the `openspec` workflow
- **THEN** exit code 1
...
- **WHEN** user runs `kfg run -w dev opencode`
...
- **WHEN** user runs `kfg run -k path -f file opencode`
- **THEN** exit code 2 (usage error)
