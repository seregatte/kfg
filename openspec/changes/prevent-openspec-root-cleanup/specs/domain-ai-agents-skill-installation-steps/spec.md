...
#### Scenario: OpenSpec cache miss preserves session planning data
- **WHEN** `openspec.steps.install` executes on a cache miss and the agent subsequently creates files under `openspec/changes/`
- **THEN** final workflow cleanup SHALL preserve the `openspec/` root and the newly created planning files
- **AND** cleanup SHALL remain limited to exact generated integration artifacts registered by the installation session

#### Scenario: OpenSpec cache hit preserves session planning data
- **WHEN** `openspec.steps.install` restores its generated integration files from cache and the agent subsequently creates files under `openspec/changes/`
- **THEN** cache restore SHALL NOT register the `openspec/` root as a directory artifact
- **AND** final workflow cleanup SHALL preserve the root and the newly created planning files
