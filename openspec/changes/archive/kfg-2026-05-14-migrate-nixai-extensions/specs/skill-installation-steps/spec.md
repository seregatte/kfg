...
#### Scenario: openspec skill installation
- **WHEN** the workflow invokes `kfg.extension.openspec.install` with `TOOLS_FLAG="--tools=claude"` and `AGENT_HOME=".claude"`
- **THEN** the Step executes `openspec init --tools=claude --force` and copies skills and commands to the output directory
